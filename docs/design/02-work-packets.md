...
| P53 | Bot turn search | web | §3, §6.2 (read) | P11, P21, P30 | **[packet](./packets/P53-bot-turn-search.md).** The heuristic bot could not perceive SPEC §3: in a committed 6-seat match half of all turns contained a *shuttle* (split, walk both parts one arrow onto the same destination, re-merge) and no seat's allowance ever exceeded its head count. Cause is per-step greedy choice — striding only beats shuttling after two of the bot's own plies. Ships a `chooseTurn` seam (`greedy-v1` frozen as baseline, `beam-v1` live), beam search over whole turn plans with `endTurn` as a searched decision, `findings.ts` demoted to move ordering, a symmetric size-scaled mobility term, and `pnpm bots` + a head-to-head test. |
